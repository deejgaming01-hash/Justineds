...
async function startServer() {
  ...
  // Vite middleware for development (Skip on Vercel as it handles static files)
  if (isMain) {
...
export default async function handler(req: any, res: any) {
  try {
    ...
    res.status(500).json({ error: "Vercel Handler Error", details: error?.message || String(error) });
  }
}
